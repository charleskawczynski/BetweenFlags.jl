    ...
    for (trig,code_substr,ft) in data
      if trig==code_substr
        ...
        elseif start_cond(ft)
          ...
          if ft isa GreedyType
            token_stream[trig2ID[trig]][i:end] .=
              max.(token_stream[trig2ID[trig]][i:end], 1)
          end
...
function get_string(code, token_stream, flag_id, cond=x->x>=1)
  i_code = [i for i in 1:length(code) if cond(token_stream[flag_id][i])]
  return code[i_code]
end
